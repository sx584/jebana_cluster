...
class jebana_FE_02(IStrategy):
    ...
    atr_mult = DecimalParameter(default=1.5, low=0.5, high=2.0, decimals=2, space='sell')
    rr_target = DecimalParameter(default=1.5, low=0.5, high=4.0, decimals=2, space='sell')
    ...
    enable_guard_metric = CategoricalParameter([True, False], default=True, space="buy")
    enable_bb_check = CategoricalParameter([True, False], default=False, space="buy")
    enable_squeeze = CategoricalParameter([True, False], default=True, space="buy")
    
    entry_guard_metric = DecimalParameter(-0.8, -0.2, default=-0.3, decimals=1, space="buy")
    ...
    def populate_entry_trend(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # atr_sl 
        df['atr_short_sl'] = df['close'] + (df['atr'] * float(self.atr_mult.value))
        df['atr_long_sl'] = df['close'] - (df['atr'] * float(self.atr_mult.value))

        # atr_tp1
        df['atr_short_tp1'] = df['close'] - (df['atr'] * float(self.atr_mult.value) * float(self.rr_target.value))
        df['atr_long_tp1'] = df['close'] + (df['atr'] * float(self.atr_mult.value) * float(self.rr_target.value))
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        model_cond = (
            # prediction crossed target
            qtpylib.crossed_below(dataframe[prediction_col], dataframe["target_loss"])
        )
