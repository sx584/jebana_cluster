...
class jebanaCS(IStrategy):
    ...
    def populate_entry_trend(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        if self.enable_bb_check.value:
            ...
            upper_limit = df["%%-bb_middleband"] + self.entry_bb_factor.value * (
                df["%%-bb_upperband"] - df["%%-bb_middleband"]
            )
            df["bearish"] = np.where((df["close"] >= upper_limit), 1, 0)
            guard_conditions.append(df["bearish"] > 0)
        ...
        if guard_conditions:
            df.loc[reduce(lambda x, y: x & y, guard_conditions), "sell_region"] = 1
            ...
            model_cond = (
                ...
                & (
                    ...
                    qtpylib.crossed_below(df[prediction_col], df["target_profit"])
                )
            )
        else:
