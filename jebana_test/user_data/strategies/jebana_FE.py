...
class jebana_FE(IStrategy):
    ...
    mom_min_roc = DecimalParameter(
        0.05, 1.50, default=0.25, decimals=2, space="buy"
    )  # % über mom_roc_win
    mom_min_bbexp = DecimalParameter(
        0.00, 0.50, default=0.10, decimals=2, space="buy"
    )  # min. BB-Expansion (relativ)
    mom_vol_mult = DecimalParameter(
        1.00, 5.00, default=1.50, decimals=2, space="buy"
    )  # Volumen / rolling-Vol
    mom_use_donch = CategoricalParameter(
        [True, False], default=True, space="buy"
    )  # Donchian-Breakout als Gate
    mom_gate_model = CategoricalParameter(
        [True, False], default=False, space="buy"
    )  # optional: auch ML-Threshold erzwingen
    ...
    plot_config = {
        "main_plot": {
            "ce_long": {"color": "#ff0000", "type": "line"},
            "atr_long_tp1": {"color": "#95a362"},
            "atr_long_sl": {"color": "#ff0000", "type": "line"},
        },
        "subplots": {
            "predictions": {
    ...
    def populate_entry_trend(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        if cw and cw > 4:
            ...
            gamma = (
                (pred_std / real_std.replace(0.0, np.nan))
                .replace([np.inf, -np.inf], np.nan)
                .fillna(1.0)
            )
            gamma = gamma.clip(
                lower=float(self.calib_gamma_min.value), upper=float(self.calib_gamma_max.value)
            )

            # Optional: Rohwert behalten für Diagnose
            df["target_profit_raw"] = df["target_profit"]
            df["target_profit"] = (df["target_profit"] * gamma).clip(
        ...
        if self.mom_enable.value:
            ...
            df["%-bb_exp"] = (
                df["%%-bb_width"].pct_change().replace([np.inf, -np.inf], 0.0).fillna(0.0)
            )
        ...
        model_cond = (df["close"] > df["ce_long"]) & qtpylib.crossed_above(
            df[prediction_col], df["target_profit"]
        )
    ...
    def order_filled(self, trade: Trade, order: "Order", current_time: datetime, **kwargs) -> None:
        ...
        try:
            ...
            if getattr(order, "ft_order_side", None) == "entry":
                ...
                short_tp1 = c["close"] - (
                    float(c["atr"]) * float(self.ce_mult.value) * float(self.rr_target.value)
                )
                long_tp1 = c["close"] + (
                    float(c["atr"]) * float(self.ce_mult.value) * float(self.rr_target.value)
                )
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        short_tp1 = signal_candle["close"] - (
            float(signal_candle["atr"]) * float(self.ce_mult.value) * float(self.rr_target.value)
        )
