...
class jebana_cali(IStrategy):
    ...
    # Einfache Hysterese um Schwellen: Entry braucht Überschreiten von (target+eps),
    # Exit braucht Unterschreiten von (target-eps). Macht die Trigger robuster gegen Rauschen.
    hyst_eps = DecimalParameter(0.0, 1.0, decimals=2, default=0.15, space="buy")
    ...
    gain_based_exit = CategoricalParameter([True, False], default=False, space="buy")
    ...
    def _val(self, name: str, default=None, cast=None):
        """Liest Strategy-Parameter robust aus (unterstützt *Parameter.value* und nackte Defaults)."""
        if hasattr(self, name):
            obj = getattr(self, name)
            v = getattr(obj, "value", obj)   # falls IntParameter/FloatParameter → .value
        else:
            v = default
    ...
    def populate_entry_trend(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        """Entry-Signale basierend auf FreqAI Vorhersagen im FE-Stil"""
        ...
        # Realisierte Gewinne (über Label-Periode) als Basis für Targets
        ...
        # Optional: Quantils-Blend (robuster Tail-Schutz)
        if bool(self.qth_enable.value):
            try:
                q = float(self.qth_quantile.value)
            except Exception:
                q = 0.75
            q = min(max(q, 0.60), 0.95)
        ...
        # Amplitudenabgleich via Gamma (Pred-Std zu Realized-Std)
        try:
        ...
        # Optionales Momentum-Gating (gleich wie in FE)
        if self.mom_enable.value:
            ...
            mom_conditions = (
                ...
                & (don_break if self.mom_use_donch.value else (df['close'] > df['ce_long']))
            )
        ...
        # Hysterese-Trigger: zuvor klar unter (target - eps), jetzt klar über (target + eps)
        eps = float(self.hyst_eps.value)
        tp_lo = (df["target_profit"] - eps)
        tp_hi = (df["target_profit"] + eps)
        prev_pred = df[prediction_col].shift(1)

        crossed_up_hyst = (prev_pred <= tp_lo.shift(1)) & (df[prediction_col] >= tp_hi)

        # Model-Trigger: Hysterese-Kreuzung + Chandelier-Gate
        model_cond = (
            (df['close'] > df['ce_long'])
            & crossed_up_hyst
        )
        ...
        # Curr-Target für Bestätigungslogik
        curr_target = 0.0
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.gain_based_exit.value:
            return df

        prediction_col = '&s-gain'
        eps = float(self.hyst_eps.value)
        tl_lo = (df["target_loss"] - eps)
        tl_hi = (df["target_loss"] + eps)
        prev_pred = df[prediction_col].shift(1)
        crossed_down_hyst = (prev_pred >= tl_hi.shift(1)) & (df[prediction_col] <= tl_lo)
        model_cond = crossed_down_hyst
        conditions.append(model_cond)

        df.loc[model_cond, "exit_tag"] += "model_exit "
        if conditions:
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        hit = (current_rate >= tp1 and not trade.is_short) or (current_rate <= tp1 and trade.is_short)
        if not hit:
            return None
