...
from freqtrade.strategy import DecimalParameter, IntParameter, CategoricalParameter, timeframe_to_prev_date, Order, stoploss_from_absolute, stoploss_from_open, timeframe_to_minutes
...
class jebana_ATP_MOMs(IStrategy):
    ...
    mom_vol_mult  = DecimalParameter(1.00, 5.00, default=1.50, decimals=2, space="buy")  # Volumen / rolling-Vol
    mom_use_donch = CategoricalParameter([True, False], default=True, space="buy")       # Donchian-Breakout als Gate
    mom_gate_model= CategoricalParameter([True, False], default=False, space="buy")      # optional: auch ML-Threshold erzwingen
    ...
    # Leverage
    leverage_amount = IntParameter(low=1, high=5, default=3, space="buy")
    ...
    entry_guard_metric = DecimalParameter(-0.8, -0.2, default=-0.5, decimals=1, space="buy")
    ...
    cexit_min_profit_th = DecimalParameter(0.0, 1.5, default=0.5, decimals=1, space="buy")
    ...
    def _prep_micro(self, pair: str, timeframe: str) -> pd.DataFrame:
        ...
        if "trades_count" in out.columns:
            ...
            out["%-trades_mean"] = out["trades_count"].rolling(tw, min_periods=max(3, tw//3)).mean()
            out["%-trades_ema"] = out["trades_count"].ewm(span=emaT, adjust=False).mean()
            out["%-trades_momo"] = (out["%-trades_ema"] / out["%-trades_ema"].shift(emaT) - 1.0).fillna(0.0)
        ...
        if "oi" in out.columns:
            ...
            out["%-oi_pct"] = (out["%-oi"] / out["%-oi"].shift() - 1.0).replace([pd.NA, pd.NaT], 0).fillna(0.0).clip(-1, 1)
    ...
    def _merge_micro(self, df: pd.DataFrame, pair: str) -> pd.DataFrame:
        ...
        fillcols = [c for c in ["trades_count", "%-trades_sum", "%-trades_mean", "%-trades_ema", "%-trades_momo", "%-trades_z",
                                "%-oi", "%-oi_delta", "%-oi_pct", "%-oi_ema", "%-oi_momo", "%-oi_z"] if c in out.columns]
    ...
    def _load_liq_df(self, pair: str) -> pd.DataFrame:
        ...
        df["%-liq_buy_ratio"] = (df["side_buy"] / df["%-liq_tot"].where(df["%-liq_tot"] > 0, 1)).fillna(0.0)
        df = df.sort_values("date").reset_index(drop=True)
        
        return df[["date", "%-liq_tot", "%-liq_notional", "%-liq_z", "%-liq_buy_ratio", "side_buy", "side_sell"]]
    ...
    def _merge_liq(self, df: pd.DataFrame, pair: str) -> pd.DataFrame:
        ...
        if self.timeframe in ("5m", "5T"):
            ...
            out = pd.merge_asof(
                ...
                on="date", direction="backward", tolerance=pd.Timedelta("10m")
            )
        
        fillcols = ["%-liq_tot", "%-liq_notional", "%-liq_z", "%-liq_buy_ratio", "side_buy", "side_sell"]
        out[fillcols] = out[fillcols].fillna(0.0)
        return out
    ...
    def feature_engineering_standard(
        ...
        bgain = 100.0 * (dataframe["close"] - dataframe["close"].shift(lookahead)) / dataframe["close"].shift(lookahead)
        ...
        if hasattr(self, 'runmode') and self.runmode in ('hyperopt', 'backtest', 'plot'):
            dataframe['%-dwt'] = dataframe['%-mid'].rolling(window=self.startup_candle_count).apply(self.roll_get_dwt)
        else:
        ...
        dataframe['%%-profit_threshold'] = dataframe['%-dwt_profit_mean'] + self.ts_n_profit_std.value * abs(
            dataframe['%-dwt_profit_std'])

        dataframe['%%-loss_threshold'] = dataframe['%-dwt_loss_mean'] - self.ts_n_loss_std.value * abs(dataframe['%-dwt_loss_std'])
        ...
        if self.use_micro.value:
            ...
            if "%-atr" in dataframe.columns:
                dataframe["%-trades_rel_atr"] = dataframe["%-trades_sum"] / (dataframe["%-atr"].replace(0, 1e-9))
                dataframe["%-oi_rel_atr"] = dataframe["%-oi"] / (dataframe["%-atr"].rolling(20).mean().replace(0, 1e-9))
        ...
        if self.use_liq.value:
            ...
            if "%-atr" in dataframe.columns:
                dataframe["%-liq_rel_atr"] = dataframe["%-liq_notional"] / (dataframe["%-atr"].replace(0, 1e-9))
            if "volume" in dataframe.columns:
                dataframe["%-liq_rel_vol"] = dataframe["%-liq_notional"] / (dataframe["volume"].rolling(20).mean().replace(0, 1e-9))
        ...
        for base, relcol in [
            ...
            if base in dataframe and relcol in dataframe:
                dataframe[relcol] = safe_div(dataframe[base].astype('float64'), atr_safe.astype('float64'))

        heavy_cols = [c for c in ['%-trades_sum','%-oi','%-oi_delta','%-oi_ema','%-liq_notional'] if c in dataframe]
        for c in heavy_cols:
        ...
        rel_cols = [c for c in ['%-trades_rel_atr','%-oi_rel_atr','%-liq_rel_atr'] if c in dataframe]
        for c in rel_cols:
    ...
    def set_freqai_targets(self, df: pd.DataFrame, metadata: dict, **kwargs) -> pd.DataFrame:
        ...
        df['future_gain'] = (
            100.0 * (df["close"].shift(-lookahead) - df["close"]) / df["close"]
        )
    ...
    def populate_entry_trend(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # Long-Conditions
        don_break_long = df['close'] > df['%%-dc_upper']
        mom_conditions_long = (
            ...
            (don_break_long if self.mom_use_donch.value else (df['close'] > df['ce_long']))
        )

        if self.mom_gate_model.value:
            mom_conditions_long &= (df[prediction_col] >= df['target_profit'])

        # Short-Conditions (umgekehrte Logik)
        don_break_short = df['close'] < df['%%-dc_lower']
        mom_conditions_short = (
            (df['%-roc_mom'] <= -float(self.mom_min_roc.value)) &  # Negative ROC für Short
            (df['%-bb_exp']  >= float(self.mom_min_bbexp.value)) &  # BB-Expansion bleibt gleich
            (df['%-vol_spike'] >= float(self.mom_vol_mult.value)) &  # Volumen-Spike bleibt gleich
            (don_break_short if self.mom_use_donch.value else (df['close'] < df['ce_short']))
        )

        if self.mom_gate_model.value:
            mom_conditions_short &= (df[prediction_col] <= df['target_loss'])  # Negative Vorhersage für Short

        # Long Entry Conditions
        conditions_long = [df["volume"] > 1.0, mom_conditions_long]

        # Buy/Sell regions (für Plotting)
        df.loc[mom_conditions_long, "buy_region"] = 1
        df.loc[mom_conditions_short, "sell_region"] = 1

        # Model triggers für Long
        if self.mom_enable.value:
            ...
            model_cond_long = qtpylib.crossed_above(df[prediction_col], df["target_profit"])

        # Model triggers für Short
        if self.mom_enable.value:
            model_cond_short = (
                (df["sell_region"] > 0) &
                qtpylib.crossed_below(df[prediction_col], df["target_loss"])
            )
        else:
            model_cond_short = qtpylib.crossed_below(df[prediction_col], df["target_loss"])

        conditions_long.append(model_cond_long)
        
        # Short conditions: nur Momentum hinzufügen wenn mom_enable = True
        if self.mom_enable.value:
            conditions_short = [df["volume"] > 1.0, mom_conditions_short, model_cond_short]
        else:
            conditions_short = [df["volume"] > 1.0, model_cond_short]
        
        # Entry-Signale setzen
        if conditions_long:
            df.loc[reduce(lambda x, y: x & y, conditions_long), "enter_long"] = 0
        
        if conditions_short:
            df.loc[reduce(lambda x, y: x & y, conditions_short), "enter_short"] = 1

        # Entry-Tags setzen
        df.loc[model_cond_long, "enter_tag"] += "model_long "
        df.loc[model_cond_short, "enter_tag"] += "model_short "
        ...
        for i in range(len(df["close"])):
            ...
            elif df["enter_short"].iloc[i] > 0.0:
                curr_target = df["close"].iloc[i] * (1.0 + df[prediction_col].iloc[i] / 100.0)  # Negative prediction für Short
    ...
    def order_filled(self, trade: Trade, order: 'Order', current_time: datetime, **kwargs) -> None:
        ...
        try:
            ...
            if getattr(order, "ft_order_side", None) == "entry":
                ...
                # TP1-Berechnung korrigiert für Short-Trades
                if trade.is_short:
                    # Short TP1: Preis geht runter (Gewinn bei fallenden Preisen)
                    tp1 = c['close'] - (float(c['atr']) * float(self.ce_mult.value) * float(self.rr_target.value))
                    sl_a = float(short_sl)
                else:
                    # Long TP1: Preis geht hoch (Gewinn bei steigenden Preisen)
                    tp1 = c['close'] + (float(c['atr']) * float(self.ce_mult.value) * float(self.rr_target.value))
                    sl_a = float(long_sl)
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        # Robust: exakten Treffer versuchen, sonst letzte Zeile <= signal_time
        sig_rows = dataframe.loc[dataframe['date'] == signal_time]
        if sig_rows.empty:
            sig_rows = dataframe[dataframe['date'] <= signal_time].tail(1)
        if sig_rows.empty:
            return None
        signal_candle = sig_rows.iloc[-1].squeeze()
        
        # TP1-Berechnung korrigiert für Short-Trades
        if trade.is_short:
            # Short TP1: Preis geht runter (Gewinn bei fallenden Preisen)
            tp1 = signal_candle['close'] - (float(signal_candle['atr']) * float(self.ce_mult.value) * float(self.rr_target.value))
        else:
            # Long TP1: Preis geht hoch (Gewinn bei steigenden Preisen)
            tp1 = signal_candle['close'] + (float(signal_candle['atr']) * float(self.ce_mult.value) * float(self.rr_target.value))
        ...
        # Keine weitere Reduktion nach dem ersten TP1
        if trade.nr_of_successful_exits >= 1:	
            return None
        
        # TP1-Hit-Logik korrigiert
        if trade.is_short:
            # Short: TP1 erreicht wenn current_rate <= tp1 (Preis ist gefallen)
            hit = current_rate <= tp1
        else:
            # Long: TP1 erreicht wenn current_rate >= tp1 (Preis ist gestiegen)
            hit = current_rate >= tp1
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
            ...
            if init_sl_abs is None:
                ...
                if not row.empty:
                    ...
                    if sl_col in c and pd.notna(c[sl_col]):
                        ...
                        # Fallback: ATR-basierte SL-Berechnung
                        atr_val = float(c.get('atr', 0.0))
                        if atr_val > 0:
                            if trade.is_short:
                                init_sl_abs = float(c['close']) + (atr_val * ch_k)
                            else:
                                init_sl_abs = float(c['close']) - (atr_val * ch_k)
                            trade.set_custom_data('init_sl_abs', init_sl_abs)
        ...
        dist = stoploss_from_absolute(stop_abs, current_rate, is_short=trade.is_short, leverage=trade.leverage)
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # R:R-basierter Exit für Long und Short
        target_profit = float(self.rr_target.value) * risk_pct
        
        if current_profit >= target_profit and current_profit > 0:
            return "rr_tp"
...
def diag_nonfinite(df: pd.DataFrame, csv_path: str = "/home/olav/freqai_diag_window.csv", row_window: int = 3):
    ...
    log.warning(f"[DIAG] Non-Finites in {len(bad_cols)} Spalten: {bad_cols[:10]}{' ...' if len(bad_cols)>10 else ''}")
